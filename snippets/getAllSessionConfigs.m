function sessionConfigs = getAllSessionConfigs()
    ...
    sessionConfigs('M18b') = SessionConfig('Leg/leg_2009_03_18/b',70,2,80,200).noiseMask(@(x,y) y>=60).pcShapeMask(@(x,y) y<50);
    sessionConfigs('M18c') = SessionConfig('Leg/leg_2009_03_18/c',70,2,80,400).noiseMask(@(x,y) y>=60).manualPeak(39,14).pcShapeMask(@(x,y) y<50);
    sessionConfigs('M18d') = SessionConfig('Leg/leg_2009_03_18/d',70,2,80,600).noiseMask(@(x,y) y>=60).manualPeak(41,15).pcShapeMask(@(x,y) y<50);
