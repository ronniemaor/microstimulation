...
% Output:
%   res: R2 value
    %res = newPerTrialVersion(yTest,yFit);
    res = newVersion(yTest,yFit);
    %res = galVersion(yTest,yFit);
    %res = corrVersion(yTest,yFit);
end
...
function res = galVersion(yTest,yFit)
    SStot = sum( (yTest(:) - mean(yTest(:))) .^2 );
    SSerr=0;
    for iPoint=1:size(yTest,2)
       SSerr = SSerr + sum((yTest(:,iPoint)-yFit(iPoint)) .^2);
    end
    res = 1 - SSerr/SStot;
end
